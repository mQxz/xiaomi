...
requirejs(["jquery","pb"],function($,pb){
	...
	$("#LoutiNav li:not(.last)").click(function(){
		...
		var _top = $(".floors").eq( $(this).index() ).offset().top;
	...
	$(window).scroll(function(){
		...
		$floor = $(".floors").filter(function(){
			return Math.abs( $(this).offset().top - sTop ) < $(this).outerHeight()/2;
		})
		var index = $floor.index();//某个楼层的下标
		$("#LoutiNav li").eq(index).find("span").addClass("active").end().siblings().find("span").removeClass("active");
	})
	
	$(".search-txt").focus(function(){
		$(".search-hint").find("a").css("display","none");
	}).blur(function(){
		$(".search-hint").find("a").css("display","block");
	})
